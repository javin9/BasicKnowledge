...
import Vue from 'vue';
	/*APP判断  */
     var isApp = tools.getCookie("YiXinAppInfo"),
        ...
        appSource;
    if (browserTerminal.versions.ios || browserTerminal.versions.iPhone || browserTerminal.versions.iPad) {
    	appSource = 896;
    }else if (browserTerminal.versions.android) {
    	appSource = 895;
    }
    if(isApp){
    	...
    	$(".det_bot a").attr("href","javascript:void(0);").on('click', function(event) {
    		event.preventDefault();
    		tools.jsNativeBridge("sendToUsedCarList",{source:appSource});//app交互
    	});
		/////判断是app用新接口
		...
		bindEvent: function(){
			...
			$('#btnEvaluate').on('click', function () {
			    if (applyCountToday < 10) {
			        // if (from != '') {
			        //     window.location.href = '/c2btf/index?again=1' + '&from=' + from + '&source=' + source;
			        // } else {
					window.location.href = indexUrl;
					// }
				   // window.location.href = '/c2btf/index?again=1' + '&source=' + source + '&from=' + from;
				} else {
					// 重新评估超出限制
