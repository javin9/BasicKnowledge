...
XTK.Action.register('GET-RULES', function (element) {
    ...
    this.render_rules = function (container) {
        var html = ` <div class="${this.style}">
                    ...
                    <p class="fw-b">详细规则：</p>
                    <p class="fw-b">活动对象：</p>
                    <p>在易鑫车贷网站或App成功贷款的用户。</p>
                     <p class="fw-b">产品范围：</p>
                    <p>易鑫自营金融产品。即：金融产品标题后缀有“易鑫集团”字样的产品。例如：一证贷 (易鑫集团) 、两证通 (易鑫集团)。</p>
                    <p class="fw-b">发放激活机制：</p>
                    <p>优惠卡：您将“优惠卡”分享给好友，TA点击领取后，TA的账户可获得价值为1000元京东E卡，在易鑫集团个人中心查看。首次还款后可激活使用（激活后会正常显示账号和密码）；
    ...
    this.render_no_product = function (container) {
        if (this.isYao) {
            var html = `  <div class="no-data"></div>
                    ...
                    <span class="desc">邀请未注册好友，当TA成功放款提车，<br />
                    即可以获得2000元的京东E卡</span>
                    <a href="${window.MyCouponCard.InviteLinkUrl}" class="btn">立即邀请</a>
                    </div>` ;
        } else {
            var html = `  <div class="no-data"></div>
                    <div class="no-data-text">
