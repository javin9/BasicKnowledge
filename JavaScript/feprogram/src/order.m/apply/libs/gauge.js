...
(function() {
  ...
  ValueUpdater = (function() {
    ...
    ValueUpdater.prototype.update = function(force) {
      ...
      if (force || this.displayedValue !== this.value) {
        ...
        if (Math.abs(diff / this.animationSpeed) <= 9) {
          this.displayedValue = this.value;
        } else {
		  if(diff>this.animationSpeed){
  ...
  GaugePointer = (function(_super) {
    ...
    GaugePointer.prototype.render = function() {
	  ...
	  //this.canvas.width=$(window).width()-30;
	  //this.canvas.height=241;
      angle = this.gauge.getAngle.call(this, this.displayedValue);
      centerX = this.canvas.width / 2;
      centerY = this.canvas.height * 0.5;
      this.x = Math.round(this.gauge.w + this.length * Math.cos(angle));
      this.y = Math.round(this.gauge.h + this.length * Math.sin(angle));
  ...
  Gauge = (function(_super) {
    ...
    Gauge.prototype.paddingBottom = 0.34;
    ...
    Gauge.prototype.setOptions = function(options) {
      ...
      this.radius = this.canvas.height * (1 - 0.5) - this.lineWidth;
      this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
      this.render();
      _ref1 = this.gp;
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
    ...
    Gauge.prototype.set = function(value) {
	  var _this=this;
	  this.yImg.onload = function(){  
		  _this.yImgLoaded=true;
		  var i, max_hit, val, _i, _j, _len, _ref1;
		  if (!(value instanceof Array)) {
			value = [value];
		  }
		  if (value.length > _this.gp.length) {
			for (i = _i = 0, _ref1 = value.length - _this.gp.length; 0 <= _ref1 ? _i < _ref1 : _i > _ref1; i = 0 <= _ref1 ? ++_i : --_i) {
			  _this.gp.push(new GaugePointer(_this));
			}
		  }
		  i = 0;
		  max_hit = false;
		  for (_j = 0, _len = value.length; _j < _len; _j++) {
			...
			if (val > _this.maxValue) {
			  _this.maxValue = _this.value * 1.1;
			  max_hit = true;
			}
			_this.gp[i].value = val;
			_this.gp[i++].setOptions({
			  maxValue: _this.maxValue,
			  angle: _this.options.angle
			});
		  }
		  _this.value = value[value.length - 1];
		  if (max_hit) {
			if (!_this.options.limitMax) {
	  ...
	  this.yImg.src = '//img1.yixinfinance.com/jinrong/assets/order.m/apply/components/images/sqdk11.html.png';
		
    };

    Gauge.prototype.getAngle = function(value) {
      return (1 + this.options.angle) * (Math.PI-0.42) + ((value - this.minValue) / (this.maxValue - this.minValue)) * (1 - this.options.angle * 2) * (Math.PI+0.84);
    };
    ...
    Gauge.prototype.render = function() {
	  ...
	  //alert(this.canvas.width);
	  this.canvas.width=$(".assessment-wrapper").width();
	  this.canvas.height=6.666667*parseInt($("html").css("font-size").replace("px",""));
	  this.w = this.canvas.width / 2;
	  //alert(".sped").width();
      this.h = this.canvas.height * (1 - this.paddingBottom);
	  w = this.canvas.width / 2;
	  //alert(".sped").width();
      h = this.canvas.height * (1 - this.paddingBottom);
	  //alert(this.canvas.height+","+this.canvas.width);
      ...
      $(this.options.changeDomId).html(parseInt(this.displayedValue)+"<em>元</em>");
      this.ctx.strokeStyle = fillStyle;
      this.ctx.beginPath();
      this.ctx.arc(w, h, this.radius, (1 + this.options.angle) * Math.PI-0.42, displayedAngle, false);
      this.ctx.lineWidth = this.lineWidth;
      this.ctx.stroke();
      this.ctx.strokeStyle = this.options.strokeColor;
      this.ctx.beginPath();
      this.ctx.arc(w, h, this.radius, displayedAngle, (2 - this.options.angle) * Math.PI+0.42, false);
      this.ctx.stroke();
      _ref1 = this.gp;
      _results = [];
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
	  ...
	  //var angle = this.getAngle(this.displayedValue);
      //centerX = this.canvas.width / 2;
      //centerY = this.canvas.height * 0.9;
      //x = Math.round(centerX + 0.1 * Math.cos(angle));
      //y = Math.round(centerY + 0.1 * Math.sin(angle));

	  if(this.yImgLoaded){
		  this.ctx.save();
		  this.ctx.translate(gauge.x, gauge.y);
		this.ctx.rotate(-0.5*Math.PI-0.42+displayedAngle-Math.PI+0.42);
		this.ctx.translate(-gauge.x, -gauge.y);
		this.ctx.drawImage(this.yImg,gauge.x-6,gauge.y-15,12,15);
		this.ctx.restore();
	  }
  ...
  window.AnimationUpdater = {
    ...
    run: function() {
      ...
      if (!animationFinished) {
		...
		// clearInterval(pgxx);
		// $(".ass-btn.font-nav").html("继续");
    if(window.gaugeCallBacks!="undefined"){
      window.gaugeCallBacks();
    }
