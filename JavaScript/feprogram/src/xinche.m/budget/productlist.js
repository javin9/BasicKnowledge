...
var productsUrl = APIURL + "api/FinancialProduct/SearchFinancialProducts",
    ...
    channel = tools.getUrlParam("channel");
var vmlist = new Vue({
    el: '#list',
    ready() {
        this.domInit();
    ...
    methods: {
        ...
        getListData() {
            ...
            tools.$ajax({
                url: productsUrl + '?carPrice=' + self.carPrice + '&carId=' + car.carId + '&cityId=' + city.cityId + '&downPaymentRate=' + self.downPaymentPercent + '&repaymentPeriod=' + self.repaymentPeriod + '&ChannelCode=&sortName=' + self.sortType + '&pageIndex=' + self.pageIndex + '&pageSize=' + self.pageSize + '&isNeedTop=true',
                type: 'Get',
                dataType: 'jsonp',
                success: function (res) {
                    ...
                    if (res.RowCount > 0) {
                        ...
                        $.each(newData, (index, val) => {
                            (function (index, val) {
                                $.ajax({
                                    url: ADVISERAPIURL + "v2/group/getadviserlist?CityId=" + city.cityId + "&CompanyIds=" + val.CompanyId,//
                                    type: 'Get',
                                    dataType: 'jsonp',
                                    async: true,
                                    cache: true,
        ...
        applyNow(item) {
            ...
            var $group = $(item).parents('.pro-groups'),
                ...
                Idx = $group.data('idx');
            window.location.href = loanOrderApplyUrl + "/common/preorder?orderId=" + tools.getUrlParam("orderId") + "&orders=" + Orders;
        },
        //点击立即申请按钮
        clickApplyNow() {
            var self = this;
...
window.onload = function () {
    ...
    // 导航条后退
    $('#header-bar .back-btn').on('click', function () {
        vmlist.goBack();
    })
