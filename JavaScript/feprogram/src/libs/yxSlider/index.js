...
export default class YXSlider{
	...
	nouisilderInit(){
		...
		this.ele.innerHTML = '';	// edit by qianyuan, 2016/12/6, 清空dom
        ...
        // 初始化高亮加cur
        $(this.sliderPips).find('li').eq(this.start).addClass('cur');

        //添加浮层
        this.sliderTips = document.createElement('ul');
		this.sliderTips.className = 'noUi-tips';
		$(this.ele).find('.noUi-handle').append(this.sliderTips);
	...
	bindEvents(){
		this.ele.noUiSlider.on('change', ( values, handle )=>{
			...
			// console.log(this.sliderDataTextArr.indexOf(parseFloat(values[handle]))>=0)
			...
			if(itemIndex<0){
				...
				// console.log("bala"+this.start)
				//修改标红项
				$(this.sliderPips).find('li').eq(this.itemSilderIndex).addClass('cur').siblings('li').removeClass('cur');
			}

			setTimeout(()=>{
				$(this.sliderTips).hide();
			}, 300)
			
		});

		//标注显示
        this.ele.noUiSlider.on('start', ( values, handle )=>{
            // console.log(this.unit)
            $(this.sliderTips).text(values[handle]<0?'不限':parseFloat(values[handle])+this.unit).show();
        });
        //标注显示
		this.ele.noUiSlider.on('slide', ( values, handle )=>{
			// console.log(this.unit)
			$(this.sliderTips).text(values[handle]<0?'不限':parseFloat(values[handle])+this.unit).show();
		});
	...
	render(){
		...
		if(this.dataLength == 1){
			...
			if(this.dataArr[0].isDisable && !this.dataArr[0].isDefault){
			...
			}else if(this.dataArr[1].isDisable && !this.dataArr[1].isDefault){
            ...
            if(this.dataArr[1].isDisable && this.dataArr[1].isDefault){
                sliderClassNameR = 'disable cur';
            }
            if(this.dataArr[0].isDisable && this.dataArr[0].isDefault){
                sliderClassNameL = 'disable cur';
            }
			...
			for (let i = 0; i < this.dataLength; i++) {
                ...
                if(this.dataArr[i].isDisable && this.dataArr[i].isDefault){
                    sliderClassName = 'disable cur';
				}
                ...
                this.sliderPipsHtml+=`<li style="left:${sliderPCT * i}%" class="${sliderClassName}">${sliderPipsItemText}</li>`;
			}
