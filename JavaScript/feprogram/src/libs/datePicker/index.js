    ...
    DatePicker.prototype = {
        ...
        bindEvent: function(type) {
            ...
            function popupDate(e) {
                ...
                _self.gearDate.innerHTML = '<div class="gearDate-con"></div><div class="date_ctrl slideInUp">' +
                    '<div class="date_btn_box">' +
                    '<div class="date_btn lcalendar_cancel font-nav">取消</div>' +
                    '<div class="lcalendar_title"></div>'+
                    '<div class="date_btn lcalendar_finish font-nav">确定</div>' +
                    '</div>' +
                    '<div class="date_roll_mask font-nav">' +
                    '<div class="date_roll">' +
                    '<div>' +
                ...
                $(_self.gearDate).find(".gearDate-con").removeClass('fade_out').addClass('fade_in');
                dateCtrlInit();
            ...
            $(document).off("click").on("click",".gearDate",function(e){
                e.stopPropagation();
                if($(e.target).hasClass('gearDate-con')){
                    ...
                    switch (target.dataset.datetype) {
                        ...
                        case "date_mm":
                            ...
                            if (stopGear) {
                                ...
                                // console.log(pos +"__" +minM +"++"+Math.abs(pos - 8))
                                setGear(target, gearVal);
                                clearInterval(target["int_" + target.id]);
                            }
                            break;
            ...
            function closeMobileCalendar(e) {
                ...
                if(!_closeCalendar){
                    ...
                    _self.gearDate.querySelector(".date_ctrl").className = "date_ctrl slideInDown";

                    $(_self.gearDate).find(".gearDate-con").removeClass('fade_in').addClass('fade_out');
                    setTimeout(function(){
                        document.body.removeChild(_self.gearDate);
                        _closeCalendar = false;
                    },300)
                }
            ...
            function finishMobileDate(e) {
                ...
                var _obj={};
                _obj.year = (date_yy % passY + _self.minY);
                _obj.month = date_mm;
                _obj.day = date_dd;
                // console.log(_self.gearDate.querySelector(".date_dd").getAttribute("val"))
                if(!isNaN(date_dd) && !isNaN(date_yy) && !isNaN(date_mm)){
                    _self.opts.CallBacks(_obj);
                    // _self.trigger.value =  + "-" +  + "-" + ;
                    closeMobileCalendar(e);
                }
            ...
            function finishMobileYM(e) {
                ...
                if(!isNaN(date_mm) && !isNaN(date_yy)){
                    _self.opts.CallBacks(_obj);
                }
            ...
            function finishMobileDateTime(e) {
                ...
                _obj.month = date_mm;
                _obj.day = date_dd;
                _obj.hour = (time_hh.length < 2 ? "0" : "") + time_hh;
                _obj.minute = (time_mm.length < 2 ? "0" : "") + time_mm;

                if(!isNaN(date_mm) && !isNaN(date_yy) && !isNaN(time_hh) && !isNaN(time_mm) && !isNaN(date_dd)){
                    _self.opts.CallBacks(_obj);
                }
                

                // _self.trigger.value =  + "-" + date_mm + "-" + date_dd + " " +  + ;
                closeMobileCalendar(e);
            }
            //时间确认
            ...
            $(_self.trigger).off("click").on("click",
                    {
                    "ym": popupYM,
                    "date": popupDate,
