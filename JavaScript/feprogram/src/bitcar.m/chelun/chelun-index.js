        ...
        if(!$fBtn.hasClass('disable')){
            ...
            function checkValidateCode(successFunc) {
                ...
                $.ajax({
                    ...
                    //添加埋点
                    try {
                        bc.evt.send('mobile', 'chkcode',$("#Telphone").val())
                    }
                    catch (err) { }
                        if (respond.Result) {
                            successFunc();
    ...
    submitOrder: function() {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                try {
                    //添加埋点
                    bc.evt.send('mobile', 'sbtclk',$('#Telphone').val(), '1', '车轮App对接H5订单提交成功')
                } catch (err) { }
                ...
                try {
                        //添加埋点
                        bc.evt.send('mobile', 'sbtclk', $('#Telphone').val(), '0', '车轮App对接H5订单提交失败 原因' + res.Message)
                    } catch (err) { }
