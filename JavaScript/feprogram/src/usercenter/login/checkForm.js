...
CheckFormPage.prototype = {
        ...
        checkForm: function() {
            ...
            $("input").on('blur', function() {
                ...
                if (fId == 'tab1' || fId == 'tab2' || fId == 'regForm') {
                    ...
                    } else if (inputId == 'ImgValidateCode2' || inputId == 'ImgValidateCode1') {
            ...
            function blurFunc($input) {
                ...
                switch (inputName) {
                    case 'mobile':
                        ...
                        // checkInput(check.isPhoneNumber(inputValue), $input, inputName);
                        break;
                    case 'mobile1':
                        newCheckInput(check.isPhoneNumber(inputValue), inputValue, inputName);
                        // checkInput(check.isPhoneNumber(inputValue), $input, inputName);
                        break;
                    case 'password':
                        //checkInput(check.isPassword(inputValue), $input, inputName);
                        let length = $("#password").val().length;
                        if (!isNaN($("#password").val())) {
                            $("#password").addClass("unsubmit nocheck").parents("#form").find(".jyts").show().text("密码不能为纯数字，8-20位");
                            checkedNumDom.hide();
                            return;
                        }
                        if (length > 20) {
                        ...
                        if (length >= 8) {
                            ...
                            $("#password").removeClass("unsubmit").parents("#form").find(".jyts").hide();
                            checkedNumDom.show();
                        }
                        break;
                ...
                function getCodeWithToken(seconds, tel_id, gvc_id, line) {
                    if (seconds <= 0) {
                        alert(倒计时时间不能小于0);
                        return false;
                    };
                    ...
                    var telNum = $("#" + tel_id).val();
                    if (check.isPhoneNumber(telNum)) {
                        tools.$ajax({
                            url: CODE_GETTING_URL,
                            data: AddAntiForgeryToken({ mobile: telNum, line: line, codelen: 4 })
                        });
                ...
                //     getCodeWithToken(60, "mobile", "GetValidateCode", BusinessLine);
                ...
                $("#GetValidateCode").click(function() {
                        ...
                        if (!$('#ImgValidateCode2Box').hasClass('hide')) {
                            if ($("#ImgValidateCode2").val() === "")
                                $("#ImgValidateCode2").focus();
                            else
                                $('#ValidateCode').focus();
                        } else {
                            $('#ValidateCode').focus();
                        ...
                        function checkLogCode() {
                            check.newGetCode({
                                ...
                                type: 'login',
                                captcha: $('#ImgValidateCode2').val()
                            }, function(res) {
                                if (!res.Result && res.IsNeedCaptcha) {
                                    // 如果token1天内连续请求第六次
                                    $('#ImgValidateCode2Box').removeClass('hide');
                                    if ($("#ImgValidateCode2").val() === "") {
                                        $("#ImgValidateCode2").focus();
                                    }
                                    if (res.RowCount == -1) {
                                        // $('#GetImgValidateCode2').find('img').attr('src', '/Login/GetImageValidateCode?t=' + (new Date().getTime()));
                                        newCheckImgFunc($("#ImgValidateCode2").val(), $("#ImgValidateCode2"));
                                    }
                                    // else if(res.RowCount==0){
                                    clearInterval(tmo);
                                    $("#GetValidateCode").addClass('disable').html('获取验证码');
                                    $('#GetImgValidateCode2').find('img').attr('src', '/Login/GetImageValidateCode?t=' + (new Date().getTime()));
                                    // }
                                } else if (res.Result) {
                                    newCheckInput($("#mobile").val(), 'mobile');
                                } else if (!res.Result && !res.Date) {
                                    tools.showAlert(res.Message);
                                }
                            });
                        }
                        let isNoImgCode = !$('#ImgValidateCode2Box').hasClass('hide');
                        if (isNoImgCode) {
                            newCheckImgFunc($("#ImgValidateCode2").val(), $("#ImgValidateCode2"));
                            if (!$("#ImgValidateCode2").hasClass('nocheck')) {
                                checkLogCode();
                            }
                        } else {
                            checkLogCode();
                ...
                $('#GetImgValidateCode2').click(function() {
                    $(this).find('img').attr('src', '/Login/GetImageValidateCode?t=' + (new Date().getTime()));
                });

                $('#changeImgValidateCode2').click(function() {
                    $('#GetImgValidateCode2 img').attr('src', '/Login/GetImageValidateCode?t=' + (new Date().getTime()));
                });

                // 注册获取验证码
                $("#GetRegValidateCode").click(function() {
                    var tel = $.trim($("#mobile").val());
                    var imgValidateCode = $.trim($("#ImgValidateCode1").val());
                    let ishasImgCode = !$('#ImgValidateCode2Box').hasClass('hide');
                    if ($(this).hasClass("disable")) {
                        return false;
                    };
                    if (!$('#ImgValidateCode2Box').hasClass('hide')) {
                    ...
                    function checkRegCode() {
                        check.newGetCode({
                            ...
                            if (!res.Result && res.IsNeedCaptcha) {
                                ...
                                // else if(res.RowCount==0){
                                clearInterval(tmo);
                                $("#GetRegValidateCode").addClass('disable').html('获取验证码');
                                $('#GetImgValidateCode2').find('img').attr('src', '/Login/GetImageValidateCode?t=' + (new Date().getTime()));
                                // }
                            } else if (res.Result) {
                                newCheckInput($("#mobile").val(), 'mobile');
        ...
        submitFunc: function(formID, url, callback) {
            ...
            function submitAjax() {
                ...
                // 	sunmitSign = $("#"+formID+" input.unsubmit").length === 1 && $('#ImgValidateCode2').hasClass('nocheck')?true:false;
                // }else {
                // 	sunmitSign = $("#"+formID+" input.unsubmit").length === 0?true:false;
                // }
            ...
            function successOrder(res) {
                ...
                if (res.Result) {
                    ...
                    // if(res.Message =="图片验证码错误"){
                    // if (formID == 'tab2' || formID == 'regForm') {
                    //   $("#"+formID+" #GetImgValidateCode2 img").attr('src', '/Login/GetImageValidateCode?t='+ (new Date().getTime()));
                    // 	$("#"+formID+" #ImgValidateCode2").addClass("nocheck");
                    // }else {
                    ...
                    checkCodeWithToken($("#ValidateCode").val(), "mobile", BusinessLine, function(res) {
                        if (!res.Result) {
                            if ($("#" + formID).hasClass("sideTip")) {
                                ...
                                // if (formID == 'tab2' || formID == 'regForm') {
                                //      	$("#"+formID+" #GetImgValidateCode2 img").attr('src', '/Login/GetImageValidateCode?t='+ (new Date().getTime()));
                                // 	$("#"+formID+" #ImgValidateCode2").addClass("unsubmit nocheck");
                                // }else {
                                // 	$("#"+formID+" #GetImgValidateCode1 img").attr('src', '/Login/GetImageValidateCode?t='+ (new Date().getTime()));
                                // 	$("#"+formID+" #ImgValidateCode1").addClass("unsubmit nocheck");
                                // }
