...
(function ($) {
    ...
    $.fn.navigate = function (options) {
        ...
        $first.on('to', function (event, r) {
            ...
            //滚动屏幕
            $("html,body").animate({
                scrollTop: pos + offsetY + options.top
            }, options.speed);
            ...
            $leftPopup.rightSwipeAction({
                clickEnd: function (display) {
                    if (display != 'none') {
                        ...
                        $leftPopup.show();
                        setTimeout(function () { 
                        	$swipeLeft.addClass('swipeLeft-block'); setTimeout(function () { $this.clicked = false; }, 500); }, 200);
   ...
   $.fn.touchClick = function(fn){
   	  var sdate = null;
   	  var $this = this;
   	  $this.touches({touchstart:function(ev){
   	  	   sdate = new Date();
   	  ...
   	  touchend: function (ev) {
    			...
    			$this  = $(ev.target).parents('a');
    		}
   	  	   if(new Date().getTime()- sdate.getTime() < 120){
           fn.call($this,ev);
          }
      }});
